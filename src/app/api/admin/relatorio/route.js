...
function montarRelatorio(
  ...
  for (const d of despesas) {
    ...
    if (!porCategoriaDespesa[categoria]) {
      porCategoriaDespesa[categoria] = {
        categoria,
        nome:
...
async function gerarExcel(
  ...
  wsDespesas.mergeCells('A2:E2');

  wsDespesas.getCell('A2').value =
  ...
  if (!despesas.length) {
    ...
    moeda(totalRow.getCell(4));

    borda(totalRow.getCell(3));
    borda(totalRow.getCell(4));
  }
...
export async function GET(request) {
  ...
  let qPedidos =
    supabaseAdmin()
      .from('pedidos')
      .select(
        'total, desconto, pagamento, status, status_pagamento, tipo, criado_em, confirmado_em, preparo_em, pronto_em, concluido_em'
      )
      .gte(
        'criado_em',
        inicio.toISOString()
  ...
  if (despesasResultado.error) {
    console.error(
      'Erro relatório despesas:',
