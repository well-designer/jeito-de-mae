...
export async function PATCH(request) {
  ...
  const parsed = statusPedidoSchema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success) return NextResponse.json({ erro: 'dados invalidos' }, { status: 400 });

  const { id, ...patch } = parsed.data;
  const { data, error } = await supabaseAdmin()
    .from('pedidos').update(patch).eq('id', id).select().single();
